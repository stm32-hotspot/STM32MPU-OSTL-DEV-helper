...
########################################################
#           COMMON
function quick_config_common() {

    your_deploy_dir_path="/tmp/STM32MPU_BUILD/"
    your_build_subdir_path="../build"
    externaldt_path="external-dt-6.0-r0/external-dt-6.0/${MACHINE}"
    sdk_absolute_path=`ls -d /opt/st/${MACHINE}/*-openstlinux-*-yocto* 2>/dev/null | tail -1`
    [[ "x${sdk_absolute_path}" == "x" ]] && sdk_absolute_path="/opt/st/${MACHINE}/<X.X.X>-openstlinux-<X.X>-yocto-<version_name>-mpu-v<XX.XX.XX>/" 

    case ${MACHINE} in
    stm32mp1)
        your_board_name="Complete with '57f-dk2', '35f-dk' or custom name"
        your_soc_name="Complete with '5' or '3'"
        your_storage_boot_scheme_security="<Complete with '' or 'min' (for stm32mp15x)>"
        your_storage_boot_scheme_cortex_a="Complete with 'emmc', 'sdcard', 'nor' or other devices"
        ;;
    stm32mp2)
        your_board_name="Complete with '57f-dk', '57f-ev', '35f-dk', '15f-dk' or custom name"
        your_soc_name="Complete with '5', '3' or '1'"
    ...
    *)
    ...
    echo ""
    echo "# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #"
    echo "# Basic configuration (for details refer sections below):       #"
    echo "# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #"
    echo ""
    echo "sdk_absolute_path=\"${sdk_absolute_path}\""
    echo "your_board_name=\"${MACHINE}<${your_board_name}>\""
    echo "your_soc_name=\"${MACHINE}<${your_soc_name}>\""
    echo "your_storage_boot_scheme_security=\"optee${your_storage_boot_scheme_security}\""
    echo "your_storage_boot_scheme_cortex_a=\"optee-<${your_storage_boot_scheme_cortex_a}>\""
    echo "your_deploy_dir_path=\"${your_deploy_dir_path}\""
    echo "your_build_subdir_path=\"${your_build_subdir_path}\""
    echo "PARALLEL_MAKE=-j4"
    echo "externaldt_path=\"${externaldt_path}\""
    [[ $1 -eq 1 ]] && echo "your_cubemx_project_name=\"<CubeMX Project Name>\""
    echo "# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # "
    echo ""
}
...
function config_stm32mp1_externaldt() {
    ...
    echo "# force external dt path to external dt env path. Set to \"\" if unused."
...
function dump_stm32mp_config() {
    ...
    *)
    ...
    validate_stm32mp_config_variables $mx
    echo ""
    echo "source \${sdk_absolute_path}/${sdk_env_file}"
}

function validate_stm32mp_config_variables() {
    local mx=$1
    required_variables="sdk_absolute_path \
			your_board_name \
			your_soc_name \
			your_storage_boot_scheme_security \
			your_storage_boot_scheme_cortex_a \
			your_deploy_dir_path \
			your_build_subdir_path \
			externaldt_path"

    if [ $mx -eq 1 ]; then
    required_variables="$required_variables your_cubemx_project_name"
    fi
    ...
    echo "for variable in \$required_variables; do"
    echo "    declare -n a=\"\$variable\""
    echo "    [[ \`echo \$a | grep -c '<'\` -gt 0 ]] && echo -e \"\\n  Missing or wrong variable assignment:\\t \$variable=\$a\\n  Please correct\\n\" && exit 1 "
    echo "    [[ \`echo \$a | grep -c '>'\` -gt 0 ]] && echo -e \"\\n  Missing or wrong variable assignment:\\t \$variable=\$a\\n  Please correct\\n\" && exit 1 "
    echo "done"
}
########################################################
...
*)
    ...
    echo "    compile-for-fip: compile all the components needed to generate fip"
    echo "    deploy-for-fip:  deploy all the components needed to generate fip and generate fip"
...
echo -e "\n"
echo -e "\t Successful generation of $OUTPUT_SCRIPT"
echo -e "\t Go up one folder, edit the script (populating configuration variables), then run it"
echo -e "\n"
