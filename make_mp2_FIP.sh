...
###                                                           ###
##                                                             ##
##  EXTERNAL DEVICETREE FOLDER generated by CubeMX:            ##
##                                                             ##
##  EXTDT_WORKING_DIR="CUBEPRJ-DK/CA35/DeviceTree/CUBEPRJ-DK"  ##
###                                                           ###
#################################################################
...
function do_build_optee() {
  ...
  OPTEE_EXTRA_OEMAKE_OPTs="-j${NUM_COREs} PLATFORM=${STM32MP_PLATFORM} CROSS_COMPILE_core=aarch64-ostl-linux- \
			  		  CROSS_COMPILE_ta_arm64=aarch64-ostl-linux- CFG_ARM64_core=y \
					  CFG_SCMI_SCPFW=y \
					  ARCH=arm NOWERROR=1 \
			  		  CFG_EXT_DTS=${EXTDT_DIR}/optee LDFLAGS= CFG_TEE_CORE_LOG_LEVEL=2 \
			  		  CFG_TEE_CORE_DEBUG=n CFG_${SOC_BASE}=y"
 
  for optee_cfg in ${OPTEE_TYPE} programmer-${OPTEE_TYPE}; do
     make -f ../Makefile.sdk.${STM32MP_PLATFORM} CFG_EMBED_DTB_SOURCE_FILE=${CUSTOM_DTS_NAME} EXTDT_DIR_OPTEE=optee EXTDT_DIR_OPTEE_SERIAL=optee \
						 DEPLOYDIR=${FIP_DEPLOYDIR_ROOT}/optee OPTEE_CONFIG=${optee_cfg} EXTRA_OEMAKE="${OPTEE_EXTRA_OEMAKE_OPTs}" optee
  done

  cd -

  mv ${FIP_DEPLOYDIR_ROOT}/optee/tee-pager_v2-${CUSTOM_DTS_NAME}-programmer-optee.bin ${FIP_DEPLOYDIR_ROOT}/optee/tee-pager_v2-${CUSTOM_DTS_NAME}-optee-programmer-usb.bin
  mv ${FIP_DEPLOYDIR_ROOT}/optee/tee-header_v2-${CUSTOM_DTS_NAME}-programmer-optee.bin ${FIP_DEPLOYDIR_ROOT}/optee/tee-header_v2-${CUSTOM_DTS_NAME}-optee-programmer-usb.bin
  mv ${FIP_DEPLOYDIR_ROOT}/optee/tee-pageable_v2-${CUSTOM_DTS_NAME}-programmer-optee.bin ${FIP_DEPLOYDIR_ROOT}/optee/tee-pageable_v2-${CUSTOM_DTS_NAME}-optee-programmer-usb.bin
}
