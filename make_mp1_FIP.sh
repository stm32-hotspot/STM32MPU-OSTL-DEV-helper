...
#################################################################
### CUSTOM EXTERNAL DEVICETREE FOLDER:                        ###
### 
EXTDT_WORKING_DIR="STM32MPU-OSTL-DEV-helper/DEVICETREE/EXT_DTS_FOR_MY_STM32MP135F-DK"
# EXTDT_WORKING_DIR="STM32MPU-OSTL-DEV-helper/DEVICETREE/EXT_DTS_FOR_MY_STM32MP157F-DK2"
### 
###                                                           ###
#################################################################
...
if [ -z "${EXTDT_WORKING_DIR}" ]; then
   ...
   for d in tf-a optee u-boot; do
     [[ "x${d}" == "xoptee" ]] && [[ ! -d "${d}" ]] && [[ -d "optee-os" ]] && ln -s optee-os optee
...
function do_build_tfa() {
  ...
  make -f ../Makefile.sdk.${STM32MP_PLATFORM} EXTDT_DIR_TF_A=tf-a DEPLOYDIR=${FIP_DEPLOYDIR_ROOT}/arm-trusted-firmware metadata
